    ...
    <td colspan="2" rowspan="4" valign="top">      <div align="center">
	 ...
	 echo "<tr><td>$row[0]</td><td>$row[1]</td><td>$row[6]</td><td>$row[9]</td><td><form action='sappoinment4.php' method='POST'>
	   <input type='submit' value='view'>
	   	   <input type='hidden' name='t2' value='$staffid'> <input type='hidden' name='t1' value='$s3'> <input type='hidden' name='t3' value='$sp'>
  ...
  <td rowspan="3" valign="top"><div align="center">Consultation Details
    ...
    <form name="form2" method="post" action="">
      <table width="186" border="1">
        ...
        <tr>
          <td><div align="center"> <?php echo "<input type='hidden' name='t2' value='$s2'> <input type='hidden' name='t1' value='$s3'> <input type='hidden' name='t3' value='$sp'> <input type='hidden' name='t0' value='$s0'>"; ?>
        <?php
		...
		$ctime=date('h:i:sA',time());
		 ...
		 $s=mysql_query("select * from booking order by bno desc");
			  ...
			  $bno++;
			  $tno="0";
			   $s=mysql_query("select * from booking where staffID='$s2' and bdate='$bdate' order by tno desc");
			  if($row=mysql_fetch_array($s))
			  {
			   $tno=$row[7];
			   }
			   ...
			   if(mysql_num_rows($s)==0)
				 ...
				 $npatients=$row[4];
				 $s=mysql_query("select * from booking where Pid='$s0' and staffID='$s2' and bdate='$bdate'");
				 if(mysql_num_rows($s)>0)
			     echo "<b>Sorry...You are already booked";
				 else
				 {
				 $row=mysql_fetch_array($s);
				$s=mysql_query("select * from booking where staffid='$s2' and bdate='$bdate'");
				$count=0;
				while($row=mysql_fetch_array($s))
				{
				$count++;
				...
				if($count==$npatients)
				   ...
				   { 
				  $s=mysql_query("select * from booking where pid='$s0' and staffid='$s2' and bdate>='$cdate' and bno not in (select bno from consultation)");
				  if (mysql_num_rows($s)>0)
				    {
					  $row=mysql_fetch_array($s);
					  $bdate=$row[3];
			    ...
			    $s="insert into booking values('$bno','$s0','$s2','$bdate','$ctime','$cdate','$staffid','$tno')";
	...
	<?php
	 $s=mysql_query("select * from staff where staffid='$s2'");
	  while($row=mysql_fetch_array($s))
	  {
		...
		?>
    <td rowspan="2" valign="top"><form name="form1" method="post" action="sappoinment5.php">
      ...
      <p align="center">&nbsp; <?php
	  echo"<input type='hidden' name='t2' value='$staffid'> <input type='hidden' name='t1' value='$s3'> <input type='hidden' name='t3' value='$sp'>
		   <input type='hidden' name='t0' value='$s0'><input type='hidden' name='c1' value='$s1'>";
	  ?></p>
      <p align="center">&nbsp;</p>
    </form>                
    <td>  
